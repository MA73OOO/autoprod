...
def detect_clis():
    ...
    known_clis = [
        {
            ...
            "template": 'ollama run llama3 "{prompt}"',
            "ping_cmd": "ollama list",
            "auth_cmd": None # Local no necesita auth
        },
        {
            "id": "gemini",
            "name": "Google Gemini", 
    ...
    for cli in known_clis:
        if shutil.which(cli["bin"]):
            ...
            if cli["ping_cmd"]:
                try:
                    # Hacemos un ping invisible rápido (3 segundos max)
                    res = subprocess.run(
                        cli["ping_cmd"], 
                        shell=True, 
                        capture_output=True, 
                    ...
                    # Si falla, o si la salida contiene "auth", "login", "key", etc.
                    output = (res.stdout + res.stderr).lower()
                    if res.returncode != 0 or "unauthorized" in output or "login" in output or "api_key" in output:
                        is_auth = False
                except:
                    is_auth = False
...
def auth_cli(provider_id: str):
    """
    Abre una terminal externa (cmd en Windows) para que el usuario inicie sesión.
    """
    import subprocess
    import platform
    ...
    try:
        # Abrir terminal flotante en Windows
        if platform.system() == "Windows":
            subprocess.Popen(f'start cmd /k "{cmd} && echo Presiona una tecla para cerrar... && pause > nul && exit"', shell=True)
